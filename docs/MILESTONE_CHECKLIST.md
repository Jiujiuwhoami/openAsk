...
- [ ] 生成新的 Admin API Key（`sk_` 前缀 + 64 位随机 hex）
- [ ] 更新后端 `.env` 中 `API_KEY` 为新生成的值
...
- [ ] 恢复或删除被误删的 `tests/test_knowledge_service.py`
- [ ] 提交 commit 并推送到远端

### □ 3. 补多租户端到端测试

- [ ] 编写 `tests/test_tenant_e2e.py` 覆盖以下场景：
  - [ ] 创建租户 → 验证返回 TenantResponse
  - [ ] 用租户 key 调 `/api/chat` → 返回该租户知识库答案
  - [ ] 用租户 A 的 key 访问租户 B 的文档 → 返回空结果（隔离验证）
  - [ ] 租户 key 过期/禁用 → 返回 401
  - [ ] 用 admin key 创建/更新/删除租户
  - [ ] 轮换 key 后旧 key 立即失效
  - [ ] 软删除租户后不返回在活跃列表中
- [ ] 全部测试通过

### □ 4. Admin API 加 Rate Limit

- [ ] 为 `/api/admin/tenants/*` 端点加限流（建议 `5/minute`）
- [ ] 验证：连续请求超限返回 429
...
- [ ] 改造 `TenantService.get_document_count()` — 接入 Zvec 查询
- [ ] 改造 `TenantStats.total_calls/prompt_tokens/completion_tokens/cache_hit_rate` — 接入实际调用计数
- [ ] 前端统计弹窗验证数据正确

### □ 6. 租户删除语义明确

- [ ] 方案一：硬删除（`DELETE /tenants/{id}` 真正删除记录 + 知识库数据）
- [ ] 方案二：保留软删除，但 list 接口默认不加 deleted 租户，加 `?include_deleted=true` 参数可选
- [ ] 删除租户时确认是否同步删除该租户的所有文档
- [ ] 前端更新：删除操作加二次确认（显示将被删除的文档数）

### □ 7. 前端租户切换加确认弹窗

- [ ] 点击下拉切换租户时弹出确认框
- [ ] 显示即将切换到的租户名称和当前操作的后果提示
- [ ] 确认后才切换

### □ 8. 前端 Admin Key 保存后验证

- [ ] `SettingsView` 中保存 Admin Key 后，自动调 `/api/admin/tenants` 验证 key 有效性
- [ ] 验证失败弹 Toast 提示 "Admin Key 无效，请检查"
- [ ] 验证成功自动刷新租户列表
...
### □ 10. Docker Compose 统一编排

- [ ] 创建 `docker-compose.yml` 编排 openAsk + admin-panel
- [ ] 统一网络、端口、环境变量管理
